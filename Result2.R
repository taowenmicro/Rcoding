...
map$cul = map$Group %>% strsplit( "-") %>%
  sapply(`[`, 1)
map$ste = map$Group %>% strsplit( "-") %>% 
  sapply(`[`, 2)
map$plant = map$Group %>% strsplit( "-") %>% 
  sapply(`[`, 3)
map$Group = paste(map$cul,map$ste,map$plant,sep = ".")
map$gro2 = paste(map$cul,map$ste,sep = ".")
sample_data(ps0) = map

sample_data(ps0)

ps = scale.solu(ps = ps0,col.nm = "Group",solu.ck = c("MeOH.NA.NA"))
ps1 = scale.black (ps = ps,
                   black.col1 = "plant",
                   black.nm1 = "TxCtrl",
                   black.col2 = "gro2",
                   black.nm2 = c("MSSuc.ster","MS.ster","MS.nonster"))
...
#--分作物进行排序分析#----
ps = ps1 #%>% subset_samples.wt("plant","TxCtrl")
...
gnum#-设定结果保存路径
repath = "./GCMS_result_and_plot_Brachy_R2/"
