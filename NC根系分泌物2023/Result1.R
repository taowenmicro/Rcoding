...
map$plant = map$Group %>% strsplit( "-") %>% 
  sapply(`[`, 1)
map$time = map$Group %>% strsplit( "-") %>% 
  sapply(`[`, 2)
map$Group = paste(map$plant,map$time,sep = "_")
sample_data(ps0) = map
sample_names(ps0)


ps = scale.solu(ps = ps0,col.nm = "Group",solu.ck = c("MeOH blank_NA"))
ps1 = scale.black (ps = ps,
                   black.col1 = "plant",
                   black.nm1 = "TxCtrl",
                   black.col2 = "time",
                   black.nm2 = c("1d","00h","02h","04h","4d","p5h"))
...
#--分作物进行排序分析#----
sample_data(ps1)
ps = ps1 %>% subset_samples.wt("plant","Arabidopsis")
...
gnum#-设定结果保存路径
repath = "./GCMS_result_and_plot_Arabidopsis_R1/"
...
for (method in methodlist) {
  result = BetaDiv(ps = ps %>% scale_micro(), group = "Group", dist = "euclidean",
